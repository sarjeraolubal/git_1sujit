...
function set_formpostvalue(){
	...
	$tmp_amount = array_sum($wtm_amount);
	if   ($dwd_total_amount == $tmp_amount) {  }
	else {$dwd_total_amount = $tmp_amount;}
	...
	if($_POST["gm_gang_code"]){$gm_gang_code = $_POST["gm_gang_code"];}
		...
		foreach($gm_gang_code as $value){

    echo 'gang codepost = ' .$value . "<br>";

                                    }
	
  //   echo "gm_gang_code1 = " . $gm_gang_code;
//     $string=implode(",",$gm_gang_code);
  //   echo   'string = " . $string;
   ...
   //  exit();
    ...
    $tmp_count = count($gm_gang_code);
    
	echo  "total men = " . $tmp_count . "  ";
	
    $dwd_totalperson = $tmp_count;
    $dwd_matha = $dwd_total_amount / $dwd_totalperson;
...
if (count($dwd_bags)==0){ 
   ...
   */
	...
	error_log("wtm_code:-".$wtm_code[$mr_rownbr].' --- amount : '.$wtm_amount[$mr_rownbr]);
		...
		if ($wtm_amount[$mr_rownbr]==0||$wtm_amount[$mr_rownbr]==null){ 
			$wtm_amount_Error = "Work Amount can not be blank";
			if (!empty($errmsg)){$errmsg = $errmsg.", ";} $errmsg = $errmsg." Work Amount can not be blank in tab Work Details";
			$OKtoSaveIntoDB = false;
		}		
    ...
    echo "gm_gang_code2 = " . $gm_gang_code;
  ...
  //    exit();
	...
	for ($mr_rownbr = 0; $mr_rownbr < $v_totalrows; $mr_rownbr++) {
     echo "gm_gang_code for loop = " . $gm_gang_code[$mr_rownbr] .  "  ";
      ...
      //  echo "gm_gang_codeimpload1 = " . $gm_gang_code[$mr_rownbr] .  " mr_rownbr  = ". $mr_rownbr  . " ";
...
//-- For Primay Info
    ...
    $dwd_total_amount = $dwd_total_amount + $dwd_lab_amount;
	GLOBAL $dwd_totalperson;
	GLOBAL $dwd_matha;
    GLOBAL $gm_gang_code;
...
$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
try{
	$sql = 'SELECT gm_badge_no,gm_name FROM lp_gang_m WHERE 1=1';
	if ($_SESSION["uprof_ddl_byco"]=="YES"){$sql .= ' AND sys_co_code = "'.$_SESSION["uprof_ddl_byco"].'"';}
	if ($_SESSION["uprof_ddl_bycobr"]=="YES"){$sql .= ' AND sys_co_br_code = "'.$_SESSION["uprof_ddl_bycobr"].'"';}
...
try{
	...
	//-- Gang Details
	$sql = "SELECT A.*,B.gm_memcatg,B.gm_bankaccnbr,B.gm_name FROM lp_dc_gdtl A,lp_gang_m B WHERE A.dwd_tabreckey=? AND B.gm_gang_code=A.gm_gang_code AND B.gm_badge_no=A.gm_badge_no";
	$q = $pdo->prepare($sql);
	$q->execute(array($_SESSION["LASTSERIAL"]));
