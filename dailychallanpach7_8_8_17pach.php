...
<div align="right">
	...
	<input type="number" onchange="setTwoNumberDecimal" min="0.00" max="9999999999999.99" step="0.01"   name="dwd_total_amount" id="dwd_total_amount" class="form-control"  style="width:150px;display:inline;background:yellow;" maxlength="20" value="<?php echo !empty($dwd_total_amount)?$dwd_total_amount:0;?>" />

	<label for="Matha" id="label_dwd_matha" class="control-label">Matha <font color="red">*</font></label>
	<input type="number" onchange="setTwoNumberDecimal" min="0.00" max="9999999999999.99" step="0.01"     name="dwd_matha" id="dwd_matha"  class="form-control" style="width:150px;display:inline;background:yellow;" maxlength="4" value="<?php echo !empty($dwd_matha)?$dwd_matha:0;?>" />
	...
	if (mtab_rowCount >0 ){
		for(var mtab_row =0;mtab_row<mtab_rowCount;mtab_row++){
			...
			var cell1 = row.insertCell(1);
			var element1 = document.createElement("input"); element1.type = "text";
			fld_id = "gm_gang_code_"+mtab_row;
			element1.id =fld_id;
			element1.name = "gm_gang_code[]";
			element1.className = "form-control";
			element1.value = mTab_Array[mtab_row].gm_gang_code;
			var gm_gang_code_CF = "refresh_badgeno("+mtab_row+")";
			var onchangeFunction = new Function(gm_gang_code_CF);
			element1.onchange = onchangeFunction;
			cell1.appendChild(element1);
			var js_gmgangcode = mTab_Array[mtab_row].gm_gang_code;

			var cell2 = row.insertCell(2);
			var element2 = document.createElement("select");
			fld_id = "gm_badge_no_"+mtab_row;
			element2.id =fld_id;
			element2.name = "gm_badge_no[]";
			element2.className = "form-control";
			var gm_badge_no_CF = "refresh_memcatg("+mtab_row+")";
			var onchangeFunction = new Function(gm_badge_no_CF);
			element2.onchange = onchangeFunction;

			var opt = document.createElement("option");
			opt.value = mTab_Array[mtab_row].gm_badge_no;
			opt.innerHTML = mTab_Array[mtab_row].gm_name;
			element2.appendChild(opt);			
			cell2.appendChild(element2);
			
			var cell3 = row.insertCell(3);
			var element3 = document.createElement("input"); element3.type = "text";
			fld_id = "gm_memcatg_"+mtab_row;
			element3.id =fld_id;
			element3.name = "gm_memcatg[]";
			element3.className = "form-control";
			...
			var cell4 = row.insertCell(4);
			var element4 = document.createElement("input"); element4.type = "text";
			fld_id = "gm_bankaccnbr_"+mtab_row;
			element4.id =fld_id;
			element4.name = "gm_bankaccnbr[]";
			element4.className = "form-control";
	...
	<div class="col-md-7">
	<input type="text" name="dwd_slip_nbr" id="dwd_slip_nbr" class="form-control" required  onchange="check_dlychln();" style="width:150px" maxlength="8" value="<?php echo !empty($dwd_slip_nbr)?$dwd_slip_nbr:'';?>" />
	</div>
	...
	<div class="col-md-7">
	<input type="number"  min="0.00" max="9999999999999.99" step="0.01"  name="dwd_lab_amount" id="dwd_lab_amount" class="form-control" required readonly   style="width:150px" maxlength="20" value="<?php echo !empty($dwd_lab_amount)?$dwd_lab_amount:0;?>" />
	</div>
	...
	function refresh_memcatg(element_no){
		...
		var mem_tot_matha = (Number(document.getElementById('dwd_total_amount').value)/tot_gangs);
		var mem_tot_matha2 = parseFloat(parseFloat(mem_tot_matha2).toFixed(2));
		
		document.querySelector('#dwd_matha').value = mem_tot_matha2; 
	...
	function calc_Lab_Amount(){
		...
		var mem_dwd_lab_amount2 = parseFloat(parseFloat(mem_dwd_lab_amount).toFixed(2));
		
		document.querySelector('#dwd_lab_amount').value=mem_dwd_lab_amount2;
	...
	function calc_Wtm_Amount(element_no){
		...
		var mem_wtm_amount2 = parseFloat(parseFloat(mem_wtm_amount).toFixed(2));

		var reset_fld = "wtm_amount_"+element_no;
		ajax_mem_wtm_amount = document.querySelector('#'+reset_fld);
		ajax_mem_wtm_amount.value = mem_wtm_amount2;
		
		var tot_dwd_total_amount = Number(document.getElementById('dwd_total_amount').value);
		tot_dwd_total_amount = tot_dwd_total_amount+ Number(mem_wtm_amount2);
		
		var tot_dwd_total_amount2 = parseFloat(parseFloat(tot_dwd_total_amount).toFixed(2));
		document.querySelector('#dwd_total_amount').value = (tot_dwd_total_amount2);
		 
		var test_fld = document.getElementsByName('wtm_amount');
		test_fld[element_no] = (mem_wtm_amount2);
		
		var test_fld2 = document.getElemenstByName('wtm_rate');
		test_fld2[element_no] = (mem_wtm_rate);
