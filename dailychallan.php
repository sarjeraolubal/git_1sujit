...
<div align="right">
	...
	<input type="number"  min="0.00" max="9999999999999.99" step=".01"   name="dwd_total_amount" id="dwd_total_amount" class="form-control"  style="width:150px;display:inline;background:yellow;" maxlength="20" value="<?php echo !empty($dwd_total_amount)?$dwd_total_amount:0;?>" />

	<label for="Matha" id="label_dwd_matha" class="control-label">Matha <font color="red">*</font></label>
	<input type="number"  min="0.00" max="9999999999999.99" step="0.01"     name="dwd_matha" id="dwd_matha"  class="form-control" style="width:150px;display:inline;background:yellow;" maxlength="4" value="<?php echo !empty($dwd_matha)?$dwd_matha:0;?>" />
	...
	<script type='text/javascript'>  
	function setTwoNumberDecimal(digit){
	 alert(digit);
		var two_dec = parseFloat(parseFloat(digit).toFixed(2));
		return two_dec;
	}
	</script>
	...
	function refresh_memcatg(element_no){
		...
		var temp1234 = (Number(document.getElementById('dwd_total_amount').value)/tot_gangs);
		//document.querySelector('#dwd_matha').value =parseFloat(parseFloat(temp1234).toFixed(2));
		document.querySelector('#dwd_matha').value = parseFloat(parseFloat(temp1234).toFixed(2));
	...
	function calc_Wtm_Amount(element_no){
		...
		mem_wtm_amount = (mem_dwd_bags *  mem_wtm_rate) / 100 ;
		//mem_wtm_amount = (mem_dwd_bags *  mem_wtm_rate);
		var mem_wtm_amount = parseFloat(parseFloat(mem_wtm_amount).toFixed(2));
		...
		test_fld[element_no] = (mem_wtm_amount);
