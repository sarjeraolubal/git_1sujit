...
if ( !empty($_POST)){
	...
	else // validate in db
	{
			...
			if (empty($db_data['up_uid']))             
             {
				...
				*/
				...
				$_SESSION['username']       = $db_data['up_name'];
				$_SESSION['usermobile']     = $db_data['up_phone'];
				$_SESSION['useremail']      = $db_data['up_uid'];
				$_SESSION['regduser_recid'] = $db_data['up_tabreckey'];
				$_SESSION['role']           = $db_data['role'];
				$_SESSION['photoname']      =$db_data['up_photoname'];	
				$_SESSION['regduser_lastsignin'] = $db_data['up_lastaccessdate'];	
				$_SESSION['regduser_signupdt']   = $db_data['up_regdate']; //$db_data['regduser_signupdt'];

				$_SESSION['sys_co_br_code'] =  '01';
              ...
              //  $form_token = bin2hex(random_bytes(128));
              $form_token = bin2hex(mcrypt_create_iv(128, MCRYPT_DEV_RANDOM));
            // this is depricated function hence replaced  random_bytes
		...
		if ($valid){header("Location:loggeduserhome.php?role='$_SESSION[role]'");}
	}
}
